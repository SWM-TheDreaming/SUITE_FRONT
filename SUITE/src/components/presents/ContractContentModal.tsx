...
const ContractContentModalPopup: React.FC<ModalPopupProps> = ({ visible, onClose, text,contract_address,block_number,tx_fee,hashed_key }) => {
  ...
  if (!visible) {
    ...
    return (
      <View style={{marginTop : 20}}>
        ...
        <View style={{flexDirection:'row'}}>
            <Text style={SuiteRoomStyleSheet.ContractModalDetailText} numberOfLines={1}>{"해시 키 : " + hashed_key}</Text>
            <TouchableOpacity style={SuiteRoomStyleSheet.ContractModalCopyIcon} onPress={()=>handleCopyToClipboard(hashed_key)}>
                <FontAwesome name="copy" size={16} color={'#050953'} />
            </TouchableOpacity>
        </View>
